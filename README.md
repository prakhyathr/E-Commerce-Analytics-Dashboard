...
![Data Pipeline Architecture](Ecommerce-Analytics/architecture_diagram.png)
...
pip install -r Ecommerce-Analytics/requirements.txt
```

### Step 2: Run the Data Processing Pipeline
Execute the notebook runner to clean, map, and output data assets:
```bash
python Ecommerce-Analytics/generate_notebooks.py
```
This generates the `.ipynb` notebooks inside `Ecommerce-Analytics/notebooks/`, aggregates files into `Ecommerce-Analytics/data/cleaned/`, and outputs static charts inside `Ecommerce-Analytics/dashboard_screenshots/`.
...
2. Execute `sql/schema.sql` to initialize tables, relationships, and performance indexes:
   ```sql
   SOURCE Ecommerce-Analytics/sql/schema.sql;
   ```
3. Use the Python bulk ingestion script provided inside `Ecommerce-Analytics/sql/data_import.sql` (under Method B) to load the cleaned CSVs into MySQL without hitting security blockades like `local_infile`.

### Step 4: Load Views & Run Business Queries
Create analytical views for BI connectivity:
```sql
SOURCE Ecommerce-Analytics/sql/views.sql;
```
Test any of the 26 analytical queries located in `Ecommerce-Analytics/sql/business_queries.sql`.
...
Below are selected high-impact findings (the full list of 15 consulting-style recommendations is available in [business_insights_report.md](Ecommerce-Analytics/reports/business_insights_report.md)):

* **Logistics Penalizes Brand Score**: Orders delivered on time maintain a strong average rating of **4.3 stars**, whereas delayed shipments plunge to **2.2 stars**. Delayed freight represents the single largest driver of low scores.
...
The dashboard utilizes a premium **dark theme** and is divided into four functional views (detailed DAX measures are documented in [powerbi/README.md](Ecommerce-Analytics/powerbi/README.md)):
